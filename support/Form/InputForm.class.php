...
class InputForm extends FormInputType {
	...
	function generate_missing_input_message( $options = array(), $newline = '<br />' ) {

		$as_array = ( isset($options['as_array']) && $options['as_array'] ) ? true : false;
		$default_message_heading = 'The following fields were not filled out completely:';

		//
		// Deprecated call with old params
		//
		if ( !is_array($options) ) {
			$message_heading = ( $options ) ? $options : $default_message_heading;
			$options = array();
			$options['newline'] = $newline;
		}
		else {
			$message_heading = ( isset($options['message_heading']) ) ? $options['message_heading'] : $default_message_heading;
		}

		//
		// Set up our initial variables depending on if we're returning a string or an array
		//
		if ( !$as_array ) {
			$newline = ( isset($options['newline']) ) ? $options['newline'] : '<br />';
			$retval  = '';
		}
		else {
			$reval   = array();
		...
		//
		$this->missing_input_message = null;

		//
		// Get Missing values array
		//
		...
		if ( count($missing_values) ) {
			...
			foreach( $missing_values as $input_name ) {
				...
				if ( $as_array ) {
					$retval[$input_name] = "The field {$friendly_name} is required";
				}
				else {
					$retval .= ( $missing_values_message ) ? $newline : '';
					$retval .= $this->get_friendly_name($input_name);
				}
			}

			if ( !$as_array ) {
				$retval = ( $message_heading ) ? $message_heading . $newline . $retval : $retval;
			}

		}

		$this->missing_input_message = $retval;
	...
	function get_messages( $options = array() ) {
		...
		//
		// Deprecated call where $newline was the first parameter
		//
		if ( !is_array($options) ) {
			$options = array();
			$newline = ( $options ) ? $options : '<br />';
			$options['newline'] = $newline;
		}
		else {
		...
		//
		// Set up some initial variables depending on whether we're returning an array or string
		//
		$as_array = false;
		$retval   = '';
		...
		//
		// Get "general messages" (arbitrary messages set at runtime)
		//
		if ( $general_messages = $this->get_general_messages($options) ) {

			if ( !$as_array ) {
				$retval .= ( $general_messages ) ? $general_messages . $options['newline'] : '';
			}
			else {
				$retval['general'] = $general_messages;
			}
		}
		...
		if ( method_exists($this, 'invalid_input_message') ) {
			...
			if ( $msgs = $this->invalid_input_message( $options ) ) {
				if ( !$as_array ) {
					$retval .= $msgs;
				}
				else {
					$retval = array_merge($retval, $msgs);
				}
			}
		}
		else {
			$missing_input = $this->get_missing_input_message( $options ); 
			
			if ( !$as_array ) {
				 $retval .= ( $missing_input ) ? $missing_input . $options['newline'] : '';
			}
			else {
				$retval = array_merge($retval, $missing_input);
			}

		}

		if ( !$as_array ) {
			$newline = preg_quote($options['newline'], '/');
			$retval  = preg_replace("/{$newline}\$/", '', $retval);
		}
