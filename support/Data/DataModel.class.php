...
class DataModel { 
	...
	const KEY_ASSOCIATION_INCLUDE_FOREIGN_COLUMN = 'association_include_foreign';  // when deleting association links, sometimes we want to be surgical in approach
																																								// so we can include both the local & foreign columns
	...
	*/
	...
	public function determine_link_table_columns_by_rel_data( $rel_data ) {
		...
		try {
			...
			$ret[self::KEY_ASSOCIATION_INCLUDE_FOREIGN_COLUMN] = false;
		
			if ( isset($rel_data[self::KEY_ASSOCIATION_INCLUDE_FOREIGN_COLUMN]) ) {
				$ret[self::KEY_ASSOCIATION_INCLUDE_FOREIGN_COLUMN] = $rel_data[self::KEY_ASSOCIATION_INCLUDE_FOREIGN_COLUMN];
			}
	...
	*/
	...
	function add( $options = array() ) {
		...
		try {  
			...
			if ( method_exists($this, 'before_record_insert') ) {
				$this->before_record_insert( $record_row );
			}

			//
			// The authoritative alias was added by LL...strip it out of our fields
			//
			$authoritative_alias = $this->get_authoritative_id_column_alias();

			if ( isset($record_row[$authoritative_alias]) ) {
				unset($record_row[$authoritative_alias]);
			}
				...
				if ( is_array($cur_foreign_val) ) {
					foreach( $cur_foreign_val as $field_name => $field_val ) {
						...
						if ( $do_replace && !$replaced ) {
							...
							if ( $options[self::KEY_ASSOCIATION_INCLUDE_FOREIGN_COLUMN] ) {
								$del_query->where(
									 array( "{$link_table}.{$link_column_foreign} = ?", $cur_foreign_val )
								);
	...
	protected function _Add_association_links( $options = array() ) {
		...
		try {
			...
			foreach( $this->_Link_columns as $foreign_field => $field_info ) {
				...
				$options[self::KEY_ASSOCIATION_INCLUDE_FOREIGN_COLUMN] = false;

				if ( $field_info[self::KEY_ASSOCIATION_INCLUDE_FOREIGN_COLUMN] == true ) {
					$options[self::KEY_ASSOCIATION_INCLUDE_FOREIGN_COLUMN] = true;
				}
	...
	function update( $options = null ) {
		...
		if ( count($record_data) > 0 ) {
			...
			$query_obj = $db->new_query_obj();
			$link_column_included_in_update = false;

			if ( method_exists($this, 'before_record_update') ) {
				$this->before_record_update( $record_data );
			}
			...
			foreach( $record_data as $field_name => $field_val ) {
				...
				if ( !$this->table_has_column_name($field_name) ) {
					if ( isset($this->_Link_columns[$field_name]) ) {
						$link_column_included_in_update = true;
						continue;
					}
			...
			$update_columns = $query_obj->get_update_column_values();
			...
			if ( count($update_columns) > 0 ) {
				...
				//
				// It's ok to have no update columns if the link column was included in the update array, 
				// since the link column might be the only thing being updated (which will be handled below)
				//
				// Otherwise, exception
				//
				if ( !$link_column_included_in_update && $this->is_record_required() ) {
					throw new NoRecordDataException( __METHOD__ . '-no_update_data_found' );					
				}
			}			
