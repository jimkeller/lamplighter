...
class FormLayoutHelper extends FormCommon {
	...
	static $Data_label_container_class = 'data_label_container_class';
    ...
    static function Container_start( $options = array() ) {
		...
		if ( isset($options['container_id']) && $options['container_id'] ) {
				$container_id = $options['container_id'];
		}

		if ( isset( $options['container_html']) && $options['container_html'] ) {
				
				foreach( $options['container_html'] as $attribute => $value ) {
						$container_html .= $attribute . '= "'  . $value . '" ';
				}
		}
    	
		$html = '<div id="' . $container_id . '" style="' . $style . '" class="' . $class_attr . '"  ' . $html_attributes . '>' . "\n";
    ...
    static function Data_start( $options = array() ) {
  		...
  		if ( isset($options['data_id']) && $options['data_id'] ) {
  			$id_tag = 'id="' . $options['data_id'] . '"';
  		}
    	
    	$html = '<div ' . $id_tag . ' class="' . $class . '">' . "\n";
    ...
    static function Data_label_start( $options = array() ) {
		...
		$html = '<div class="' . self::$Data_label_container_class . '">' . "\n";
   ...
   public static function Password_field( $options = array() ) {
    ...
    public static function Form_component( $options = array() ) {
    	...
    	if ( isset($options['field_tag']) && $options['field_tag'] ) {
			...
			if ( !isset($sub_options['input_name']) || !$sub_options['input_name'] || !isset($sub_options['name']) || !$sub_options['name'] ) {
				$sub_options['name'] = self::Input_name_from_option_hash( $sub_options );
			}
		...
		try {
			...
			switch( $options['input_type'] ) {
				...
				case 'radio':
					...
					foreach( $options['options'] as $option_data ) {
						...
						if ( !($input_id = self::Input_id_specified($sub_options)) ) {
							$input_id =$sub_options['input_name'] . '-' . preg_replace('/[^A-Za-z0-9-\-]/', '-', $option_val); 
						} 
						...
						$output .= '<label for="' . $input_id . '_' . FormInput::$Last_radio_index . '" class="form-radio">' . $option_text . '</label>';
						$output .= "\n" . '</div>';
				...
				case 'textarea':
					...
					$output = FormInput::Text_area( $sub_options );
