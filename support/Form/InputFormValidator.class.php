...
class InputFormValidator extends InputForm {
	...
	function invalid_input_message( $options = array() ) {
		...
		//
		// Determines whether basic requirements validation (e.g. "is the field present or not"?) failed
		// Set in validate_input() 
		//
		if ( $this->check_input_failed ) {
			return $this->missing_input_message( $options );
		}
		
		//
		// Now, check the more advanced validators
		//
		$invalid_values = $this->get_invalid_input();
			...
			foreach( $invalid_values as $cur_value ) {
				...
				if ( $as_array ) {
					$retval[$input_name] = $input_message;				
				}
				else {
			            $retval .= ( $invalid_value_message ) ? $newline : '';
			            $retval .= $input_message;
			        }
			}        
	        }                            
        
		if ( $retval && !$as_array ) {
	            $retval = $this->invalid_value_heading . $newline . $retval;
		}                
